...
const AnimatedCard = ({ title, description }: { title: string, description: string }) => {
    ...
    return (
        <div className="relative flex flex-col lg:h-[36rem] z-30">
            {/* translate container */}
            <div className="h-full w-full hover:bg-black hover:opacity-90" onMouseEnter={() => settranslateY(true)} onMouseLeave={() => settranslateY(false)}>
                <div className="h-[30rem] w-full overflow-y-hidden">
                    <div className={`translate-y-[13rem] px-4 flex flex-col items-start justify-end py-2 gap-6 h-full duration-500 transition-all ease-in-out ${translateY ? 'translate-y-0' : ''}`}>
                        <h2 className="fira-sans-bold text-white text-[35px] font-[600] leading-none">{title}</h2>
                        <p >{description}</p>
                        <div className="w-full flex items-center">
                            <div className="w-32 h-12 rounded-full flex justify-center items-center bg-blue-500 shadow-custom-light">
                                <p className="text-white font-[800] fira-sans-regular">Read More</p>
                            </div>
                        </div>
                    </div>
                </div>
